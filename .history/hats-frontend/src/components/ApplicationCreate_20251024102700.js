...
export default function ApplicationCreate() {
  ...
  if (!authData || authData.user.role === 'admin' || authData.user.role === 'botmimic') {
    return <p>You do not have permission to apply.</p>;
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="m-4 p-4 border rounded" style={{ maxWidth: 500 }}>
      ...
      <textarea className="form-control mb-3" placeholder="Initial Comment (optional)" value={comment} onChange={e => setComment(e.target.value)} rows={3}></textarea>
      <button className="btn btn-primary w-100" type="submit" disabled={loading}>{loading ? 'Submitting...' : 'Submit'}</button>
