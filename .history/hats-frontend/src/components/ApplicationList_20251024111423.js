...
export default function ApplicationList() {
  ...
  const [statusEdits, setStatusEdits] = useState({}); // { appId: status }
  ...
  const handleUpdateStatus = async (appId) => {
    ...
    try {
      ...
      // refetch applications to show update
      const res = await api.get('/application/list');
      setApplications(res.data.applications);
      setStatusEdits((prev) => ({ ...prev, [appId]: undefined })); // Reset edit after update
    } catch (err) {
                    ...
                    <>
                      ...
                      <ul className="list-group list-group-flush">
                        {app.comments.map((comment, idx) => (
                          <li key={idx} className={`list-group-item ${comment.includes('Bot Mimic') ? 'text-info fst-italic' : ''}`}>
                            {comment}
                          </li>
                        ))}
                      </ul>
