      ...
      <div className="container">
        ...
        <div className="d-flex">
          {!authData ? (
            ...
            <>
              <Link to="/applications" className="btn btn-success me-2">Applications</Link>
              {userRole !== 'admin' && userRole !== 'botmimic' && (
                <Link to="/apply" className="btn btn-info me-2">Apply</Link>
              )}
              {userRole === 'admin' && (
                <>
                  <Link to="/admin/jobs" className="btn btn-primary me-2">Post Jobs</Link>
                  <Link to="/admin/applications" className="btn btn-secondary me-2">View Applications</Link>
                </>
              )}
...
const AppRoutes = () => {
  ...
  return (
    <Routes>
      ...
      <Route
        path="/apply"
        element={authData && userRole !== 'admin' && userRole !== 'botmimic' ? <JobListWithApply /> : <Navigate to="/applications" />}
      />
      <Route
        path="/admin/jobs"
      ...
      <Route
        ...
        element={authData?.user?.role === 'admin' ? <ApplicationList /> : <Navigate to="/" />}
      />
      <Route path="/jobs" element={<JobListWithApply />} />
<Route path="/admin/jobs" element={<AdminJobPost />} />
